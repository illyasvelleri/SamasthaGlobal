...
const FatwaCard = ({ fatwa }) => {
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs font-semibold px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-700 border border-emerald-200/60">
          {fatwa.category || 'Uncategorized'}
        </span>
        <div className="flex items-center gap-3 text-xs text-slate-500">
          <span>{(fatwa.views ?? 0).toLocaleString()} views</span>
          <span>•</span>
          <span>{fatwa.date || 'N/A'}</span>
        </div>
      </div>

      {/* Title */}
      <h3 className="text-lg md:text-xl font-semibold text-slate-900 mb-3 line-clamp-2 group-hover:text-emerald-700 transition-colors">
        {fatwa.title || 'No Title Available'}
      </h3>
      ...
      <div className="space-y-2 mb-5">
        <p className="text-sm font-semibold text-emerald-800">Question:</p>
        <p className="text-sm text-slate-700 line-clamp-2 leading-relaxed">
          {fatwa.question || 'Question not provided'}
        </p>
        <p className="text-sm text-slate-500 line-clamp-2 italic">
          {fatwa.excerpt || 'No summary available'}
        </p>
      </div>
...
export default function FatwasPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const data = await getFatwaData(); // should return { categories?, fatwas: [...] } or just array

        // Handle both possible return shapes
        const fatwaList = Array.isArray(data) ? data : data?.fatwas || [];
        setFatwas(fatwaList);
        setFilteredFatwas(fatwaList);
      } catch (err) {
          ...
          <section className="px-5 sm:px-8 lg:px-12 pb-10">
            <div className="max-w-6xl mx-auto">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10">
                ...
                <div className="w-full sm:w-auto">
                  <input
                    ...
                    placeholder="Search (fasting, zakat, marriage...)"
                    className="w-full px-5 py-3 bg-white/80 backdrop-blur-sm border border-slate-200 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-emerald-400"
                  />
                </div>
              </div>
              ...
              {loading ? (
                ...
                <div className="text-center py-20 text-slate-500 text-lg">
                  {searchQuery
                    ? 'No fatwas match your search.'
                    : 'No fatwas available at the moment.'}
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              ...
              {/* Optional: Load more button (static for now) */}
              {!loading && filteredFatwas.length > 0 && (
                <div className="text-center mt-12 md:mt-16">
                  <button className="px-12 py-5 rounded-2xl bg-gradient-to-r from-emerald-600 to-blue-600 text-white font-medium text-lg shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300">
                    Load More
                  </button>
